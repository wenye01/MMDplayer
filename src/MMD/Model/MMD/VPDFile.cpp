...
namespace mmd
{
    bool ReadVPDFile(VPDFile * vpd, const char * filename)
    {
        ...
        {
            MMD__INFO("VPD File Open Fail. {}", filename);
            return false;
        }
        ...
        {
            MMD__INFO("VPD File Format Error.");
            return false;
        }
        ...
        if (lineIt == lines.end())
        {
            MMD__INFO("VPD File Parse Error.[parent fil name]");
            return false;
        }
        lineIt++;
        ...
        if (lineIt == lines.end())
        {
            MMD__INFO("VPD File Parse Error.[num bones]");
            return false;
        }
        int numBones = 0;
        try
        ...
        {
            MMD__INFO("VPD File Parse Error. {}:[{}]", size_t(lineIt - lines.begin()), e.what());
            return false;
        }
        ...
        while (boneCount < numBones && lineIt != lines.end())
        {
            int boneIdx = 0;
            {
                ...
                {
                    MMD__INFO("VPD File Parse Error. {}:[Not Found Bone]", size_t(lineIt - lines.begin()));
                    return false;
                }
                delimPos1 += 4;
                ...
                if (boneIdx >= numBones)
                {
                    MMD__INFO("VPD File Parse Error. {}:[Bone Index over]", size_t(lineIt - lines.begin()));
                    return false;
                }
            ...
            
            {
                ...
                {
                    MMD__INFO("VPD File Parse Error. {}:[Split error]", size_t(lineIt - lines.begin()));
                    return false;
                }
        ...
        for (auto& bone : bones)
        {
            std::u16string u16Str = mmd::ConvertSjisToU16String(bone.m_boneName.c_str());
            std::string u8Str;
            mmd::ConvU16ToU8(u16Str, u8Str);
        ...
        while (lineIt != lines.end())
        {
            VPDMorph morph;
            {
                ...
                if (delimPos1 == line.npos)
                {
                    MMD__INFO("VPD File Parse Error. {}:[Not Found Morph]", size_t(lineIt - lines.begin()));
                    return false;
                }
        ...
        {
            std::u16string u16Str = mmd::ConvertSjisToU16String(morph.m_morphName.c_str());
            std::string u8Str;
            mmd::ConvU16ToU8(u16Str, u8Str);
