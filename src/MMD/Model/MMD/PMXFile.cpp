...
namespace mmd
{
    namespace
    {
        ...
        bool ReadMorph(PMXFile* pmx, File& file)
        {
            ...
            for (auto& morph : pmx->m_morphs)
            {
                ...
                {
                    MMD__ERROR("Unsupported Morph Type:[{}]", (int)morph.m_morphType);
                    return false;
                }
            }
        ...
        bool ReadPMXFile(PMXFile * pmxFile, File& file)
        {
            ...
            {
                MMD__ERROR("ReadMorph Fail.");
                return false;
            }

            if (!ReadDisplayFrame(pmxFile, file))
            {
                MMD__ERROR("ReadDisplayFrame Fail.");
                return false;
            }

            if (!ReadRigidbody(pmxFile, file))
            {
                MMD__ERROR("ReadRigidbody Fail.");
                return false;
            }

            if (!ReadJoint(pmxFile, file))
            {
                MMD__ERROR("ReadJoint Fail.");
                return false;
            }
    ...
    bool ReadPMXFile(PMXFile * pmxFile, const char* filename)
    {
        ...
        {
            MMD__INFO("PMX File Open Fail. {}", filename);
            return false;
        }

        if (!ReadPMXFile(pmxFile, file))
        {
            MMD__INFO("PMX File Read Fail. {}", filename);
            return false;
        }
        MMD__INFO("PMX File Read Successed. {}", filename);
