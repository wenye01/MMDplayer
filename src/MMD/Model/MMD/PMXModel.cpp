...
namespace mmd
{
    ...
    bool PMXModel::Load(const std::string& filepath, const std::string& mmdDataDir)
    {
        ...
        for (const auto& v : pmx.m_vertices)
        {
            ...
            default:
                vtxBoneInfo.m_skinningType = SkinningType::Weight1;
                MMD__ERROR("Unknown PMX Vertex Weight Type: {}", (int)v.m_weightType);
                break;
            }
        ...
        default:
            MMD__ERROR("Unsupported Index Size: [{}]", m_indexElementSize);
            return false;
        }
        ...
        for (const auto& pmxMorph : pmx.m_morphs)
        {
            ...
            {
                MMD__WARN("Not Supported Morp Type({}): [{}]",
        ...
        {
            MMD__ERROR("Create Physics Fail.");
            return false;
        }
        ...
        for (const auto& pmxRB : pmx.m_rigidbodies)
        {
            ...
            {
                MMD__ERROR("Create Rigid Body Fail.\n");
                return false;
            }
        ...
        for (const auto& pmxJoint : pmx.m_joints)
        {
            ...
            {
                MMD__WARN("Illegal Joint [{}]", pmxJoint.m_name.c_str());
            }
        }
    ...
    void PMXModel::SetupParallelUpdate()
    {
        ...
        {
            MMD__WARN("PMXModel::SetParallelUpdateCount parallelCount > {}", maxParallelCount);
            m_parallelUpdateCount = 16;
        }

        MMD__INFO("Select PMX Parallel Update Count : {}", m_parallelUpdateCount);
